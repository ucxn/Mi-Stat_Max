...
Theoretically, it supports all Wi-Fi 5/6/7 series routers running Xiaomi's stock firmware. Nearly all AX and newer models can provide comprehensive data. ISP-customized models may provide only partial data, but compatibility remains broad.

This is far more than a simple UI enhancement plugin. Powered by *Bro-Tech*'s ingenious algorithms, it performs comprehensive processing on the raw, dynamic, discrete data, continuously approaching the true values as closely as possible. It corrects the original firmware's fake WAN speed readings—which are generated by aggregating LAN-side MAC statistics—and reconstructs the actual WAN-side throughput. Public network capability always matters more than internal network statistics. It accurately analyzes device traffic distribution and integrates calculus and signal processing into the script. Only by reading the source code can one truly appreciate its ingenuity.

For a general and interesting overview, please see [Design Blueprint](https://github.com/ucxn/BroTech/blob/main/README_EN.md).
